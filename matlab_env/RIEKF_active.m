...
record = 0;
if record == 1
    video = VideoWriter(['video/' num2str(index) num2str(op) num2str(explore) num2str(id_f) '.avi']);
    video.FrameRate = 5;
    video.Quality = 100;
    open(video);

    video2 = VideoWriter(['video/env' num2str(index) num2str(op) num2str(explore) num2str(id_f) '.avi']);
    video2.FrameRate = 5;
    video2.Quality = 100;
    open(video2);
end
...
%                 end
            ...
            if (d1 < 0.5 && d1z < 0.2) || i == 2%i == 10 %|| sum(u(4:6, end) == 0)==3
                subgoal = goals(:, n);
...
%             lock = mod(lock+1, 8);
        ...
        u(:, i - 1) = getV2(2, op, x, i, subgoal, explore, map, lock,id_f);
        toc;
        tc = toc;
        t = [t; tc];
...
ef_ave = sum(ef2) / length(ef2)
    ...
    savefig(['fig/' num2str(index) 'RIEKF' num2str(op) num2str(explore) num2str(id_f) '.fig']);
                    ...
                    set(h_free, 'XData',goalx(free(1, :)), 'YData', goaly(free(2, :)), 'ZData',goalz(free(3, :)));
    savefig(['fig/' num2str(index) 'RIEKF' num2str(op) num2str(explore) num2str(id_f) '_env.fig']);
