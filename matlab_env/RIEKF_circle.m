...
record = 0;
if record == 1
    video = VideoWriter(['video/c' num2str(index) num2str(op) num2str(explore) num2str(id_f) '.avi']);
    video.FrameRate = 5;
    video.Quality = 100;
    open(video);

    video2 = VideoWriter(['video/cenv' num2str(index) num2str(op) num2str(explore) num2str(id_f) '.avi']);
    video2.FrameRate = 5;
    video2.Quality = 100;
    open(video2);
end

load(['dataFeature' num2str(id_f)], 'M','feature','feature_','tri_','tri1','goalx','goaly','goalz','unknown','vertex');
% N = 221;
N = 1000;
N_elipse = 201;
...
%         Q = sparse(blkdiag(sigma_head^2 * eye(3), sigma_odem^2 * eye(3)));
        ...
        if lock == 0 && i ~= 2
            ...
            if (d1 < 0.5 && d1z < 0.2) || i == 10 %|| sum(u(4:6, end) == 0)==3
                subgoal = goals(:, n);
    %             plot3(subgoal(1), subgoal(2), subgoal(3), 'o', 'color', 'm', 'MarkerFaceColor', 'm', 'MarkerSize', 5);
        ...
        u(:, i - 1) = getV(2, op, x, i, subgoal, explore, map, lock,id_f);
        toc;
        tc = toc;
        t = [t; tc];
        [~, Pose(:, i)] = RIEKF_predict(1, u(:,i-1), Q, P, Pose(:, i-1), zeros(6,1));
        [P_pre, x_pre] = RIEKF_predict(2, u(:,i-1), Q, P, x, noise_odem(:, i));
        %feature measurement
        k = 0;
        vis_feat_new = [];
        for j = 1:M
