function obj = RIobjFun(u0, map,id_f)
...
for jj=1:1:M
    if ismember(jj, vis_feat)
        ...
        if q <= range(2)
            flag = 1;
            xid = max(1, (x_pre(6+3*j-2)-goalx(1)+1) / (goalx(end)-goalx(1)+1) * length(goalx));
            yid = max(1, (x_pre(6+3*j-1)-goaly(1)+1) / (goaly(end)-goaly(1)+1) * length(goaly));
    ...
    N_ = sparse(diag(z_est.^2) * sigma_r^2);
        [K, H] = RIEKF_gain(x_pre, P_pre, N_, vis, M, vis_feat, k);
