...
record = 1;
if record == 1
    video = VideoWriter(['video/' num2str(index) num2str(op) num2str(explore) '.avi']);
    video.FrameRate = 5;
    video.Quality = 100;
    open(video);

    video2 = VideoWriter(['video/env' num2str(index) num2str(op) num2str(explore) '.avi']);
    video2.FrameRate = 5;
    video2.Quality = 100;
    open(video2);
end
...
%             lock = mod(lock+1, 8);
        ...
        [u(:, i - 1),ur(:, i - 1)] = getV2(2, op, x, i, subgoal, explore, map, lock,id_f, ranges, angles);
        tc = toc(t_plan);
        t = [t; tc];
...
save(['result/' num2str(index) 'RIEKF' num2str(op) num2str(explore) num2str(id_f) '.mat'], 'u', 'ur', 'traceP', 'num_unseen',...
