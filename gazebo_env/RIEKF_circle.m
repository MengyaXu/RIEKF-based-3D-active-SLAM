...
record = 1;
if record == 1
    video = VideoWriter(['video/c' num2str(index) num2str(op) num2str(explore) '.avi']);
    video.FrameRate = 5;
    video.Quality = 100;
    open(video);

    video2 = VideoWriter(['video/cenv' num2str(index) num2str(op) num2str(explore) '.avi']);
    video2.FrameRate = 5;
    video2.Quality = 100;
    open(video2);
end
...
%                 mapm = occupancyMatrix(map,'ternary'); % 0 free; 1 occ; -1 unknown
            ...
            if (d1 < 0.3 ) || i == 10 %|| sum(u(4:6, end) == 0)==3
                subgoal = goals(:, n);
...
save(['result/' num2str(index) 'RIEKFc' num2str(op) num2str(explore) num2str(id_f) '.mat'], 'u', 'ur', 'traceP', 'num_unseen',...
